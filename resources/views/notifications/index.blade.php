    ...
    @else
        <div class="space-y-3">
            @foreach($notifications as $notif)
                <div class="p-4 sm:p-5 rounded-2xl border transition-all flex items-start justify-between gap-4 {{ is_null($notif->read_at) ? 'bg-indigo-50/40 border-indigo-200/80 shadow-xs' : 'bg-white border-slate-200/80' }}">
                    <div class="flex items-start gap-3.5">
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 {{ is_null($notif->read_at) ? 'bg-gradient-to-tr from-indigo-600 to-violet-600 text-white shadow-xs' : 'bg-slate-100 text-slate-400' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                        </div>
                        <div class="space-y-1">
                            <div class="flex items-center gap-2">
                                <h4 class="font-bold text-xs sm:text-sm text-slate-900 {{ is_null($notif->read_at) ? 'text-indigo-950' : '' }}">{{ $notif->title }}</h4>
                                @if(is_null($notif->read_at))
                                    <span class="w-2 h-2 rounded-full bg-indigo-600"></span>
                                @endif
                            </div>
                            <p class="text-xs text-slate-600 leading-relaxed">{{ $notif->message }}</p>
                            <span class="text-[10px] text-slate-400 font-medium block">{{ $notif->created_at->diffForHumans() }}</span>
                        </div>
                    </div>
